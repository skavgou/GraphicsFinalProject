...
struct MyBot {
	...
	void updateSkinning(const std::vector<glm::mat4> &nodeTransforms) {
		...
		for(int i = 0; i < skinObjects.size(); ++i) {
			int rootNodeIndex = model.skins[i].joints[0];
			glm::mat4 parentTransform(1.0f);
			computeGlobalNodeTransform(model, nodeTransforms, rootNodeIndex, parentTransform, skinObjects[i].globalJointTransforms);
			for (int j = 0; j < model.skins[i].joints.size(); ++j) {
				skinObjects[i].jointMatrices[j] = skinObjects[i].globalJointTransforms[model.skins[i].joints[j]] * skinObjects[i].inverseBindMatrices[j];
			}
		}
		// -------------------------------------------------
	}
